...
def wordnet_graph(list_words):
     ...
     def recurse(word_synset):
         ...
         if not word_synset in seen:                           # if not seen...
             ...
             for synset in hypernyms:                              # for hypernyms
                 recurse(synset)                                   # do so until top
                 dot.edge( str(word_synset.name()), str(synset.name()) )
      
     # build network containing all categories          
     for word in list_words:  
         recurse(wn.synset(word))
...
if __name__ == '__main__':
    ...
    dot = Digraph('diagram', format='svg', filename=path_root+'out/img/tree_relations/relations_categories.gv')
    dot.attr(rankdir='BT') # rankdir = LR        |     size='6,6'
